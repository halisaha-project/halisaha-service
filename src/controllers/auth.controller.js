...
const register = async (req, res) => {
  ...
  return res.status(201).json({
    ...
    data: token,
    message:
      'User created successfully. Please check your e-mail and verify your account.',
  })
}
...
const confirmMail = async (req, res) => {
  ...
  const user = await User.findOne({ _id: jwtData.sub }).select(
    'verificationCode createdAt'
  )
  if (!user) throw new APIError('Invalid email or verification code.', 404)
  if (verificationCode != user.verificationCode)
    throw new APIError('Invalid email or verification code.', 400)
